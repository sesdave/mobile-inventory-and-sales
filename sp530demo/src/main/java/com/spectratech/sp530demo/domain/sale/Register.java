package com.spectratech.sp530demo.domain.sale;
...
import com.spectratech.sp530demo.domain.inventory.Stock;
...
public class Register {
	...
	public Sale initiateSal(String startTime) {
	 ...
	 */
	public LineItem addItem(String varname,Product product, int quantity) {
		if (currentSale == null)
			initiateSale(DateTimeStrategy.getCurrentTime());
		
		LineItem lineItem = currentSale.addLineItem(varname,product, quantity);
	...
	public LineItem addItem(String varname,Product product, int quantity,double varprice) {
			//initiateSal(DateTimeStrategy.getCurrentTime());
		if (currentSale == null)
			initiateSale(DateTimeStrategy.getCurrentTime());

		LineItem lineItem = currentSale.addLineItem(varname,product, quantity,varprice);
	...
	public LineItem addItem(String varname, int quantity,double varprice) {
		//initiateSal(DateTimeStrategy.getCurrentTime());
		if (currentSale == null)
			initiateSale(DateTimeStrategy.getCurrentTime());

		LineItem lineItem = currentSale.addLineItem(varname, quantity,varprice);
	...
	public int getSize() {
		if (currentSale == null) return 0;
		return currentSale.size();
	}
	public int getQuantity() {
		if (currentSale == null) return 0;
		return currentSale.getQuantity();
	}
